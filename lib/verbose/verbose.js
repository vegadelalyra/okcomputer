...
const filePath = 'howManyWords.js';
const fileUrl = new URL(filePath, import.meta.url);
const fileRealPath = fileUrl.pathname;
const fixedFileRealPath = path.normalize(fileRealPath);

import('fs').then(fs => {
    fs.promises.mkdir(path.dirname(fixedFileRealPath), { recursive: true })
        .then(() => fs.promises.writeFile(fixedFileRealPath, how_many_words))
